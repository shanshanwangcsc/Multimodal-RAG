...
This repository not only demonstrates the implementation of a multimodal RAG system, but also emphasizes scalability by leveraging Distributed Data Parallel (DDP), particularly for handling large datasets. The overal workflow is shown in the figure above.
